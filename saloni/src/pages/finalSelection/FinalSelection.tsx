...
const FinalSelection = () => {
  ...
  const checkoutHandler = async () => {
    ...
    const orderData = await axios.post(
      `${process.env.REACT_APP_BASEURL}${process.env.REACT_APP_RAZORPAY_CHECKOUT}`,
      {
        amount: 400,
      },
    );
    ...
    console.log(
      `${process.env.REACT_APP_BASEURL as string}${
        process.env.REACT_APP_RAZORPAY_VERIFICATION as string
      }`,
    );
    const options = {
      ...
      callback_url: `${process.env.REACT_APP_BASEURL as string}${
        process.env.REACT_APP_RAZORPAY_VERIFICATION as string
      }`,
      prefill: {
        name: 'Gaurav Kumar',
