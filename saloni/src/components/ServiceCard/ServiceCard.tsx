...
const ServiceCard: React.FC<ServiceCardProps> = (props) => {
  ...
  const handleAddClick = (props: ServiceCardProps) => {
    ...
    const data = { userId: user.userId, salonId: salon_id, serviceId: id };
  ...
  const handleRemoveClick = (props: ServiceCardProps) => {
    ...
    const data = {
      userId: '88109dd4-ec1b-4c44-9669-60b0e48f33c0',
      salonId: salon_id,
      serviceId: id,
    };
        ...
        >
          <CardBody pl={'20px'} pr={'40px'}>
            <HStack justifyContent={'space-between'}>
              ...
              <Flex>
                {!loader && !cart.cartList.some((item) => item.id === id) && (
                  <GrAddCircle
                    size={'30px'}
                    style={{ opacity: 0.8 }}
                    cursor={'pointer'}
                    onClick={() => handleAddClick(props)}
                  />
                )}
                {loader && (
                  <CircularProgress
                ...
                {!loader && cart.cartList.some((item) => item.id === id) && (
                  <Text color={'#60b246'}>Added to cart</Text>
                )}
              </Flex>
            </HStack>
          </CardBody>
