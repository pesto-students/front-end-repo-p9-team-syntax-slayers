...
const Cart: React.FC<CartProps> = (props) => {
  ...
  const handleClearCart = () => {
    ...
    axios
      .post(apiEndpoint, { userId: '88109dd4-ec1b-4c44-9669-60b0e48f33c0' })
      .then((res) => {
        console.log(res);
        dispatch(emptyCart());
      })
      ...
      >
        <HStack>
          <Text color={'white'} fontWeight={700}>
            {cartDetails.cartList.length}{' '}
            {cartDetails.cartList.length > 1 ? 'Services' : 'Service'} |{' '}
          </Text>
          <Text color={'white'} fontWeight={700}>
            RS.{total}
          </Text>
